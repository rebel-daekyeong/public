...
_world_size: int = int(os.environ.get("WORLD_SIZE", None) or 2)
...
class DModel:
    def __init__(self, world_size: int, rank: int, world_port: int):
        ...
        self.org_model = ToyModel().to(self.local_rank)
        ...
        for i in range(_num_iter):
            torch.manual_seed(i)
            inp = torch.rand(20, 10, requires_grad=True).to(self.local_rank)
    ...
    def get(self):
        res = str(self.tp_model(torch.rand(20, 10).to(self.local_rank)))
        print(res)
        return res
...
@serve.ingress(app)
class Gateway:
    def __init__(self, models, world_size: int, world_port: int):
        self.world_size = world_size
        self.models = models
    ...
    def get(self):
        ...
        res = self.models[0].get.remote()
        for rank in range(1, _world_size):
            self.models[rank].get.remote()
        return ray.get(res)
...
for rank in range(_world_size):
    _models.append(
        DModel.options(
            ...
            get_if_exists=True,
        ).remote(_world_size, rank, _world_port)
    )
model = Gateway.bind(_models, _world_size, _world_port)
_models = None
